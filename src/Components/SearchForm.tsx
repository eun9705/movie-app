...
const SearchForm = ({submitFn,searchType,setSearchType,searchContext,setSearchContext}:SearchFormProps) => {
    ...
    return (
        <form onSubmit={submitFn} className="search-form">
            ...
            <select name="searchType" id="searchType" onChange={(e:React.ChangeEvent<HTMLSelectElement>)=>setSearchType(e.target.value)} defaultValue={searchType}>
                <option value="title">제목</option>
                <option value="director">감독</option>
            </select>
            <label htmlFor="searchContext">검색어입력</label>
            <input type="text" placeholder="검색어를 입력해주세요." name="searchContext" required id="searchContext" onKeyPress={handlerKeyPress} onChange={(e:React.ChangeEvent<HTMLInputElement>)=>setSearchContext(e.target.value)} defaultValue={searchContext}></input>
            <button type="submit">
                <GoSearch />
            </button>
        </form>
